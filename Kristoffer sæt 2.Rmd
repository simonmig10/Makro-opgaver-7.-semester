...
Assume production function of final goods sector is given by: $$Y(t)=[(1-a_K)K(t)]^\alpha[A(t)(1-a_L)L(t)]^{1-\alpha}$$
...
Production functions of R&D sector is given by: $$\dot A(t)=B[a_K K(t)]^\beta[a_L L(t)]^\gamma A(t)^\theta$$
...
Vi starter med at opskrive produktionsfunktionen for final goods sector:
$$Y(t)=[(1-a_K)K(t)]^\alpha [A(t)(1-a_L)L(t)]^{1-\alpha}$$


Vi indsætter produktionsfunktionen i kapitalakkumuleringsfunktionen og får:
$$\dot{K}=C_K*A(t)^{1-\alpha}*K(t)^{\alpha-1}*L(t)^{1-\alpha}$$

hvor det gælder, at $s(1-a_K)^\alpha*(1-a_L)^{1-\alpha}=C_K$, så:
...
Sætter vi = 0 og fjerner $(1-\alpha)$ ved at dividere med den, så får vi $$g_A^* +n-g_K^*$$ og isoleres $g_K^*$ fåes $$g_K^* = g_A^*+n$$
